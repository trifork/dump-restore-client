 ...
 *
 *  $HeadURL: https://svn.softwareborsen.dk/dump-restore-client/tags/release-1.0.2/dump-restore-client/src/test/java/dk/nsi/dump_restore_client/TestStaticConfig.java $
 *  $Id: TestStaticConfig.java 10985 2013-04-02 20:22:06Z ChristianGasser $
 * /
 */
...
public class TestStaticConfig {
    ...
    public void testNullInputStream() {
        StaticConfigImpl.loadFromStream(null);
    }

    @Test(expected = StaticConfig.ConfigException.class)
    public void testEmptyStream() {
        StaticConfigImpl.loadFromStream(new ByteArrayInputStream("".getBytes()));
    }

    @Test(expected = StaticConfig.ConfigException.class)
    public void testMalformedConfiguration() {
        StaticConfigImpl.loadFromStream(new ByteArrayInputStream("<root/>".getBytes()));
    }
    ...
    public void testInvalidConfigurations() {
        try {
            ...
            StaticConfigImpl.loadFromStream(new ByteArrayInputStream(configString.getBytes()));
            fail();
        } catch (StaticConfig.ConfigException e) {
    ...
    public void testValidConfiguration() throws MalformedURLException {
        StaticConfig staticConfig = StaticConfigImpl.loadFromStream(TestStaticConfig.class.getResourceAsStream("/unittest-dump-restore-config.xml"));
